...
function curl1($openid)
{//curl方式通过提交openid换取PORTAL_TICKET
    $ch = curl_init();
...
function curl2($PORTAL_TICKET)
{//通过提交PORTAL_TICKET通过验证获得课表页面html数据
    $ch = curl_init();
...
function curl3($cookie, $PORTAL_TICKET)
{//curl2的前提下将得到的cookies重新提交至curl2重定向的页面，并加上Referer头
    $ch = curl_init();
...
function curl4($zc, $cookie, $PORTAL_TICKET)
{//在curl3的前提下url添加字段zs=01,02,03等来直接获取周次的课表
    $ch = curl_init();
